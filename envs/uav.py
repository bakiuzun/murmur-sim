...
class UAVEnv:
    ...
    def step(self, mjx_data, action, _internal_step, success_counter,previous_obs):
        
        
        mjx_data = mjx_data.replace(ctrl=action * 13.0)
        ...
        obs = self._get_obs(mjx_data)
        reward = compute_reward(obs,previous_obs,self.reward_config)
        previous_obs = obs
        #success_counter = jnp.where(reward >= 10.5, success_counter + 1, 0)
        #is_success = success_counter >= 200

        rotation_z = obs[8]  # fixed indexing

        done = jnp.float32(
            (_internal_step >= 1500)
            | (jnp.abs(mjx_data.qpos[0]) >= 15.0)  # x drift
            | (jnp.abs(mjx_data.qpos[1]) >= 15.0)   # y drift
            | (jnp.abs(mjx_data.qpos[2]) >= 20.0)   # z drift
            | (rotation_z < 0.) # 
            )          
            #| is_success
        ...
        _internal_step += 1
        return mjx_data, obs, reward, done, _internal_step, success_counter,previous_obs
